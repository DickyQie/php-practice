...
class SQLHelper {
    ...
    public function excute_sql_fenye($sql1,$sql2,$fengyePage){
        ...
        if($fengyePage->pageNow<$fengyePage->pageCount){
            ...
            $navigate.="<a href='{$fengyePage->gotoUrl}?pageNow=$nextPage'>下一页</a>&nbsp;";
        }

        for($i=1;$i<=$fengyePage->pageCount;$i++){
            $navigate.="<a href='{$fengyePage->gotoUrl}?pageNow={$i}'>$i</a>&nbsp;&nbsp;";
        }

        $navigate.="当前第{$fengyePage->pageNow}页共{$fengyePage->pageCount}页";



        $navigate.="<form action='{$fengyePage->gotoUrl}' method='get' onsubmit='return checkPageNow()'>跳转到<input type=\"text\" id='pageNow' name=\"pageNow\" onkeyup=\"return checkPageNow();\"><input type='submit' value='Go'></form>";
