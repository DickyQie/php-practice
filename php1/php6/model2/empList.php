...
<script type="text/javascript">
    function showClick(val){
        return window.confirm("是否要删除ID=="+val+"用户");
    }
</script>
    <?php
       ...
       for($i=0;$i<count($fenyePage->res_array);$i++){
           $row=$fenyePage->res_array[$i];
            echo "<tr><td>{$row["id"]}</td><td>{$row["name"]}</td><td>{$row["gade"]}</td><td>{$row["email"]}</td>
                        <td>{$row["salary"]}</td><td><a href=''>修改</a> </td><td><a onclick='return showClick({$row['id']})' href='empList.php?fang=delete&id={$row['id']}'>删除</a> </td></tr>";
        }
        echo "</table>";
